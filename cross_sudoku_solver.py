class CrossSudokuSolver:
    ...
    def validBoard(self):
        ...
        diagonals = [[set() for _ in range(2)]]
    ...
    def checkValid(self,input_value,x,y):
        ...
        for i in range(9):
            x_axis = x_start + i % 3
            y_axis = y_start + i // 3
            if(x >= 9 or y >= 9 or input_value == self.board[y][i] or input_value == self.board[i][x] or input_value == self.board[y_axis][x_axis] or ((x + y) == 8 and self.board[i][8-i] == input_value) or (x == y and self.board[i][i] == input_value)):
                return False
        return True
